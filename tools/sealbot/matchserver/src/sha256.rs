// Vendored verbatim from crates/pistol-cli/src/sha256.rs. The matchserver is
// deliberately outside the workspace (its Cargo.toml says why), so it cannot
// call that module; the option matrix for this reader chose a vendored
// std-only digest over pulling the CLI in
// (docs/experiments/matrix_anchor_openings_reader.md §4, option O2/A2).
// A divergence here is a bug: the two must agree byte for byte, which
// `the_vendored_digest_agrees_with_the_book_header` pins against the
// committed book's own recorded body digest.
