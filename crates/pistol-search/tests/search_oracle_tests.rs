//! The differential search oracle: `Searcher` against an independently written
//! full-width negamax over the same move universe (docs/decisions.md D-106).
//!
//! What these tests certify is SOUNDNESS — that PVS, iterative deepening, the
//! transposition table and move ordering compute the same game-theoretic values
//! as plain minimax. Everything Stage 1 adds is a pruning change, and rule 6's
//! SPRT judges strength rather than soundness, so a pruning commit that quietly
//! changed the value of the tree would pass one.
//!
//! # Two tiers, and why
//!
...
//! `perft_tests.rs` already uses for the movegen oracle (docs/decisions.md D-54,
//! D-120).
//!
//! # What is where
//!
