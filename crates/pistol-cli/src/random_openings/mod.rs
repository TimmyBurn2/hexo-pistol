...
/// The name of the v1 book. Kept as its own constant because six test files
/// pin the v1 fixture through it, and v1 is the artifact every closed SPRT
/// verdict was taken over: those tests go on pinning v1 whatever else is
/// generated (docs/decisions.md D-505).
...
pub enum BookVersion {
    /// `random_openings_v1.txt` — RETIRED FOR GOVERNED USE (docs/decisions.md
    /// D-505), still readable as the artifact the closed verdicts were taken
    /// over, and still regenerated byte for byte by its own pinning test.
    V1,
...
impl BookVersion {
    /// The file this version is written to.
    ...
    /// How this version spells itself in prose and in a test name.
