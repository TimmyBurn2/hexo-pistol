//! The relevance zone sequence `Z_1 ⊆ Z_2 ⊆ Z_3` and the RZOP operations that
//! build it.
//!
//! Wu & Lin (IEEE TCIAIG 2(3), 2010, §IV) adapted to this formulation per
//! docs/experiments/wp18a_design.md §3. The operations, by their paper labels:
//!
//! - **AT-1** — an OR node proven by move m takes its child's zone plus m's
//!   cells, at EVERY order: [`ZoneP::union_with`] followed by
//!   [`ZoneP::add_all_orders`]. Putting the attacker's own move cells in every
//!   order is what makes no tolerated defender pre-placement able to sit on
//!   one.
//! - **DT-1** — an AND node proven takes the union of its children's zones
//!   elementwise: [`ZoneP::union_with`] over the children, plus the EP-1 and
//!   T3-1 contributions of the node itself.
//! - **EP-1** — every proof node grades DEFENDER ACTIVE SEGMENTS: a 6-window
//!   with no attacker stones (any defender count including zero — the paper's
//!   §III-A definition) with exactly k unoccupied squares contributes those
//!   squares to `Z_k..Z_3` (k ≥ 3 collapses to `Z_3`, the paper's own
//!   order-3 cap for two-stone moves). [`ep1_contribution`] implements it.
//! - **T3-1** — the attacker's hot-window empties enter all orders:
//!   [`ZoneP::add_all_orders`] over `ThreatState::threat_cells`.
//!
//! # THE ONE MARKED DEVIATION FROM THE PAPER
//!
//! The paper's boards are finite; this game's is not. The active-segment scan
//! is bounded to windows intersecting the position's own legal region (the
//! radius-8 union around its stones, rule 5): cells beyond it are unreachable
//! by any legal stone at this node, and the scan re-runs at every proof node
//! as the board grows, so an outward-marching build enters the scan one step
//! behind the march — exactly when it becomes legally reachable. The legal
//! region is monotone along proof paths (stones are only added), so every
//! window intersecting the ROOT's region is scanned at every proof node,
//! which is the class gate (c)'s σ sample draws from.
//!
//! No inheritance from the paper's Theorem 2 is claimed: the property this
//! construction actually has is the one the oracle gates falsify or confirm
//! (design §3's own words).
