//! One engine subprocess, and the two ways it can stop talking.
//!
//! # Why a reader thread
//!
//! A pipe read blocks forever, and this crate must be able to say "this engine
//! answered nothing" without hanging the whole run. `std` gives no read
//! deadline, so each child gets a thread that reads lines and hands them over a
//! channel, and the arena waits on the channel with a timeout.
//!
//! # Silence is not an answer
//!
//! The timeout NEVER produces a game result. It abandons the run
//! ([`ArenaError::Hung`]), because a timeout that forfeited a game would make
//! the verdict a function of how fast this machine is — the determinism law
//! sitting inside the instrument every strength claim comes from (CLAUDE.md
//! rule 4, docs/decisions.md D-159).
//!
//! An engine that CLOSES its pipe has stopped talking for a reason the arena can
//! attribute, and the attribution is the exit status: a child that exited with a
//! CODE chose to, and that is a deterministic answer to a deterministic input,
//! so it forfeits. A child with no code was killed from outside — an OOM kill,
//! a signal — and that is the same machine-dependence wearing an EOF, so it
//! abandons the run like a hang. Without this split, running at eight workers on
//! a memory-tight box would silently score OOM kills as losses.
//!
//! # Bounded reads, and why the bound is a refusal rather than a cut
//!
//! pistol-cli bounds every line it WRITES (`MAX_REFUSAL_CHARS`, `QUOTED_CHARS`).
//! This is the mirror image for what the arena READS, so an engine emitting one
//! unterminated gigabyte cannot exhaust the referee's memory.
//!
//! What the cap must NOT do is hand the truncated chunk over as though it were a
//! line. An earlier version did, and it defeated the watchdog outright: an
//! engine streaming endless bytes that happen to begin `info ` was read as an
//! endless sequence of legitimate progress reports, so the timeout that fires
//! on silence never got the chance to, and a run could be occupied forever with
//! `hang_timeout_ms` set to a fraction of a second. A line longer than the cap
//! is now [`Received::Overlong`] — a deterministic protocol violation the
//! referee forfeits on, which is what a deterministic wrong answer earns.
