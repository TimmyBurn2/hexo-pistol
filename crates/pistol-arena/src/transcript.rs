...
pub struct RecordedGame {
    ...
    /// happened, so a replay says `Ongoing` and cannot say who was awarded it
    /// (`docs/experiments/wp20s_design.md` §4).
        ...
        games.push(RecordedGame {
            ...
            result: {
                ...
                GameResult::from_token(word).ok_or_else(|| {
                    refuse(format!(
                        "`game {index}` records `result {word}`, which is not a result this build \
                         writes"
                    ))
                })?
            },
