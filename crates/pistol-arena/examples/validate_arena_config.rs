...
fn main() -> ExitCode {
    ...
    if paths.is_empty() {
        eprintln!("validate_arena_config: give at least one path");
        return ExitCode::from(2);
    }
    let mut refused = 0;
    for path in &paths {
        match ArenaConfig::load(path) {
            Ok(_) => println!("validate_arena_config: {} ok", path.display()),
            Err(error) => {
                eprintln!("validate_arena_config: {} REFUSED: {error}", path.display());
                refused += 1;
            }
        }
    }
    if refused == 0 {
        println!("validate_arena_config: {} document(s) ok", paths.len());
