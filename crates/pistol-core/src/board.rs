...
//!
//! The board is unbounded, so occupancy is sparse: a map from cell to colour,
...
///
/// Black moves first; the first turn is one stone (rule 3).
...
impl Color {
    ...
    /// The protocol and fixture spelling of this colour.
...
impl Board {
    ...
    /// The colour of the stone on `at`, if any.
    pub fn get(&self, at: Coord) -> Option<Color> {
        self.stones.get(&at).copied()
    }

    /// Whether `at` holds a stone of either colour.
    ...
    pub fn stones(&self) -> impl Iterator<Item = (Coord, Color)> + '_ {
        self.stones.iter().map(|(&at, &color)| (at, color))
    }
    ...
    pub fn apply(&mut self, at: Coord, color: Color) -> Result<(), CoreError> {
        match self.stones.entry(at) {
            Entry::Occupied(_) => Err(CoreError::OccupiedCell { at }),
            Entry::Vacant(slot) => {
                slot.insert(color);
                self.key ^= cell_key(at, color);
                Ok(())
            }
        }
    }

    /// Take the stone off `at`, returning its colour.
    pub fn undo(&mut self, at: Coord) -> Result<Color, CoreError> {
        let color = self
            .stones
            .remove(&at)
    ...
    /// region, and it asks nothing about colour — rule 5 says *an existing
