//! Zobrist keys: the 128-bit name a position answers to.
//!
...
//!
//! A whole position key is the XOR of
//!
//! - one [`cell_key`] per stone on the board,
//! - one [`side_key`] for the side to move, and
//! - one [`phase_key`] for the intra-turn phase — a turn with one of its two
//!   stones down is not the node the completed turn is (D-9).
//!
//! XOR makes the stone part order-free, which is the whole point: two turn
//! orders that reach the same stones reach the same key, and the search sees one
//! node rather than two. It also makes the part incremental — a stone placed and
//! a stone taken back are the same XOR — which is why the board maintains it at
//! the one place stones move ([`crate::Board::apply`], D-41).
//!
//! # Construction
//!
//! Each key is two independent SplitMix64 streams over one 64-bit *word* that
...
//! # Determinism
//!
//! Nothing here is seeded at run time, nothing is cached, and nothing consults
//! the clock or the allocator: two processes, two machines and two builds
//! produce the same key for the same position (CLAUDE.md rule 4). The pinned
//! vectors in `tests/zobrist_tests.rs` are what would notice if that stopped
//! being true.
