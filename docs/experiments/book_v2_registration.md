...
**Criteria C and D are receipted in `artifacts/book_v2_power_refusals_v1.txt`**,
because a criterion recorded MET with no artifact behind it is a claim.
| B — the tilt hits its target | `tilted_t` vs `target` | equal to 6 dp at truths −25, 0, 15, 25 | **MET**, and pinned by a test |
| C — an unreachable target is refused | — | `FAIL: target t 4070.43… is outside what this bucket shape reaches ([-2287.01, 3100.76])`, exit 1 | **MET**, receipted |
| C — zero buckets, a short bucket list, zero runs, zero pairs, an unknown option, an option with no value | — | six further distinct named refusals, exit 1 each | **MET**, receipted |
| D — determinism | the same argv twice | byte-identical | **MET**, receipted |

**A FIFTH FIGURE THIS DOCUMENT DID NOT REGISTER LANDS TOO. IT IS NOT WHY
CRITERION A PASSED — the four registered figures are — and it is recorded
because of what it would have meant had they NOT.** D-187 also records that raising the cap from 500
...
constrains, from the same run. Four registered figures could in principle be hit by a model tuned to them; a
fifth unregistered one landing to within 1.2 and 2.5 pairs could not be. So the
fifth figure is evidence about the MODEL rather than about the criterion:
criterion A was met on its own four, its consequence does not fire, and the
fallback it names is not taken. Had A missed, this row is what would have said
whether the miss was a model difference or an error.
...
**Freshness is checked, not asserted**, and §6.1 says exactly what is checked:
`the_two_books_overlap_only_as_far_as_independent_drawing_makes_them` in
`crates/pistol-cli/tests/random_openings_document_tests.rs` measures the two
books' overlap — as payload lines and as positions up to symmetry — and **pins
both counts exactly**. It does not require the intersection to be empty, because
independent draws from one finite pool do not give an empty one; §6.1 has the
arithmetic and the reason a filter was declined.
...
| loadability | **every entry**, through the guarded bench block (D-475): `bench_block: done: 4500 entries x 1 reps = 4500 totals lines, 0 refused`, exit 0 — `artifacts/book_v2_loadability_v1.txt` |

### 6.0 THE LOADABILITY RECEIPT, and why it is over EVERY entry

The dispatch requires *"loadability receipt over every entry via the guarded
bench block (D-475)"*, and the reason is D-475's own: the shared bench command
block once substituted an empty board for a refused entry — engine error to
stdout, exit 0, totals from the stale position — so a sweep could report numbers
for positions it never loaded. The guard that fixed it REFUSES a run in which
any entry is refused, which is what makes `0 refused` over 4500 a receipt rather
than a silence.

```
tools/bench_block.sh --engine target/release/pistol \
  --config configs/instrument_staged_v0.toml \
  --fixture crates/pistol-cli/tests/fixtures/random_openings_v2.txt \
  --grammar tail --budget 'nodes 1' --reps 1 --label book-v2-loadability
```

`nodes 1` because the question is whether the engine can take the position, not
what it thinks of it: the budget is the smallest one that still exercises
`position` and `go` on every line.
