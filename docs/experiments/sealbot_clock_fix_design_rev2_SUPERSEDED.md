# Design — make the sealbot seat's measured wall mean its search time. **REVISION 2, SUPERSEDED**

**SUPERSEDED by revision 3.** Its REVIEW-design returned **FAIL**, 6 MAJOR and
8 MINOR. Criterion (1), Change A's premise and the A1/A4 adjudications all HELD
under the reviewer's own measurement; what fell was §1's arithmetic (two
start-up constants 4x apart for the same run, and a looseness ratio computed on
a referent the same section disowns nine lines later) and criteria (2), (3) and
(4) — (4) being unable to fire at all while (3) holds, which is the vacuity
round 1 killed, recommitted in the criterion this document called its
centrepiece.
