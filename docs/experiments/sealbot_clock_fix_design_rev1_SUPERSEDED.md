# Design — make the sealbot seat's measured wall mean its search time — **REVISION 1, SUPERSEDED**

**SUPERSEDED by `sealbot_clock_fix_design.md` revision 2.** Its REVIEW-design
returned **FAIL** with 7 MAJOR and 10 MINOR. The measurement in §1 survived — a
reviewer re-took it on 51 genuinely early-returning positions this document's own
table omits — but **§4's criteria did not**: criterion (1) passed on fully
defective data at every ODD game count, criterion (3) was failed by the seat that
already works, and none of the three excluded a shim reporting its CONFIGURED
budget. Kept because the ADR owes the strongest surviving attack.
